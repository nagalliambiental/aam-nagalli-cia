...
const COR = (natureza: string, status: string) => {
  if (status === "concluido" || status === "concluida") return "#2f8ac1";
  if (status === "em_andamento" || status === "ativo") return "#24678f";
  return "#ff740d";
};
...
export function GanttPrazos({ barras }: { barras: Barra[] }) {
  ...
  if (escala === "mes") return { start: inicioSemana(new Date(cursor.getFullYear(), cursor.getMonth(), 1)), end: fimSemana(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 0)) };
    if (escala === "trimestre" || escala === "semestre") {
      const primeiroMes = Math.floor(cursor.getMonth() / 3) * 3;
      const inicio = escala === "semestre" ? Math.floor(cursor.getMonth() / 6) * 6 : primeiroMes;
      return { start: new Date(cursor.getFullYear(), inicio, 1), end: fimMes(cursor.getFullYear(), inicio + (escala === "semestre" ? 5 : 2)) };
    }
  ...
  const unidades = useMemo<Unidade[]>(() => {
    ...
    if (escala === "mes") {
      for (let start = rangeStart; start <= rangeEnd; start += UM_DIA) { const d = new Date(start); output.push({ label: d.toLocaleDateString("pt-BR", { day: "2-digit" }), start, end: fimDia(d).getTime() }); }
    } else {
      const months = escala === "ano" ? 12 : escala === "semestre" ? 6 : 3;
  ...
  const titulo = escala === "mes"
    ...
    : escala === "trimestre"
      ...
      : escala === "semestre"
        ? `${Math.floor(cursor.getMonth() / 6) + 1}º semestre de ${cursor.getFullYear()}`
  ...
  const navegar = (passos: number) => setCursor((atual) => { const d = new Date(atual); if (escala === "ano") d.setFullYear(d.getFullYear() + passos); else if (escala === "semestre") d.setMonth(d.getMonth() + passos * 6); else if (escala === "trimestre") d.setMonth(d.getMonth() + passos * 3); else d.setMonth(d.getMonth() + passos); return d; });
  const zoom = (direction: number) => { const niveis: Escala[] = ["mes", "trimestre", "semestre", "ano"]; const atual = niveis.indexOf(escala); setEscala(niveis[Math.max(0, Math.min(niveis.length - 1, atual + direction))]); };
  ...
  return <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
      <div className="flex flex-wrap items-center gap-1">
        <button type="button" onClick={() => navegar(escala === "ano" ? -1 : escala === "semestre" ? -2 : escala === "trimestre" ? -4 : -12)} title="Anterior" className="flex h-8 w-8 items-center justify-center rounded-md bg-navy-700 text-white hover:bg-navy-800"><ChevronsLeft className="h-5 w-5" /></button>
        ...
        <button type="button" onClick={() => navegar(escala === "ano" ? 1 : escala === "semestre" ? 2 : escala === "trimestre" ? 4 : 12)} title="Próximo" className="flex h-8 w-8 items-center justify-center rounded-md bg-navy-700 text-white hover:bg-navy-800"><ChevronsRight className="h-5 w-5" /></button>
