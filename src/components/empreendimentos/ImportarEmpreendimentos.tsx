...
export function ImportarEmpreendimentos() {
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/empreendimentos/importar", { method: "POST", body: fd });
      const d = await res.json().catch(() => ({}));
      if (res.ok) {
        const err = d.erros?.length ? ` | ${d.erros.length} erro(s): ${d.erros.slice(0, 3).join("; ")}` : "";
        setMsg(`${d.criados} empreendimento(s) importado(s).${err}`);
      } else {
